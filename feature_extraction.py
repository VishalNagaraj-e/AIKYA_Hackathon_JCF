...
def compute_feats(image, kernels):
    ...
    for k, kernel in enumerate(kernels):
        filtered = np.convolve(image, kernel, mode='wrap') #changed nd to np here
...
def GLCM_Feature(cropped):
    ...
    glcm = graycomatrix(cropped, [1, 2], [0, np.pi/2], levels=256, normed=True, symmetric=True)  # graycomatrix instead of greycomatrix
