...
global hr_status
hr_status= 0  #variable for determining hr status
global eye_status
eye_status = 0 #variable for eye data
exec(open("app_file.py").read())
...
class Window(Frame):
    ...
    def Train_Eye_Movement(self, event=None):
        ...
        def train_CNN(data,label):
            ...
            history = model.fit(np.array(X_train), np.array(X_label), epochs=20, #epochs changed to 2 from 20
                                validation_data=(np.array(Y_test), np.array(Y_label))) #added np.array to the y_label part
    ...
    def Handwriting_Prediction(self, event=None):
        ...
        T.insert(END,contents)
        print(contents)
        if contents == 'Dyslexia Handwriting':
            hr_status = 1
...
if hr_status == 1 and eye_status == 1:
    file.write("positive,positive,high")
elif hr_status == 1 and eye_status == 0:
    file.write("positive,negative,moderate")

elif hr_status == 0 and eye_status == 1:
    file.write("negative,positive,moderate")
else:
    file.write("negative,negative,low")



file.close()
exec(open("writing_to_sql.py").read())
